...
export const adminRouter = createTRPCRouter({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const updatePayload: Partial<typeof configTable.$inferInsert> = {
        ...
        bookingBufferMinutes:
          input.bookingBufferMinutes ?? existing.bookingBufferMinutes,
  ...
  createSlot: managerProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const slotTemplate = createSlotFromConfig(
        input.date,
        input.from,
        input.to,
        config.slots,
  ...
  deleteSlot: managerProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      return {
        success: true,
        message: "Slot deleted. Virtual slot will now be used.",
      };
    }),
...
export const superAdminRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [updated] = await db
        ...
        .set({
          ...
          flatDiscountAmount:
            input.flatDiscountAmount ?? coupon.flatDiscountAmount,
          ...
          firstNBookingsOnly:
            input.firstNBookingsOnly ?? coupon.firstNBookingsOnly,
  ...
  couponStats: superAdminProcedure.query(async () => {
    ...
    const totalUsage = allCoupons.reduce(
      (sum, c) => sum + (c.numberOfUses ?? 0),
      0,
    );
    const totalCapacity = allCoupons.reduce(
